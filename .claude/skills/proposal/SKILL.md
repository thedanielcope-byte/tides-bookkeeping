...
- **Pricing model:** two-phase — a **one-time cleanup/catch-up** + a **monthly** service (month-to-month). The cleanup is **one payment at signing**. Only split it 50% deposit / 50% on completion when the cleanup fee is **over $3,000**. Monthly begins on a clean **go-live date** (usually the 1st of the next month). Cleanup covers through the prior period; monthly covers go-live forward — **state the no-overlap explicitly** so the client never asks "am I paying twice?".
